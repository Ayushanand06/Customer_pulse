...
export function PredictionResult({ prediction }: PredictionResultProps) {
  ...
  const getAdvice = (score: number, category: string) => {
    if (score >= 80) {
      ...
      return "Good satisfaction level. Monitor for improvement opportunities and gather specific feedback."
    } else {
        ...
        <div className="text-center">
          <div className="relative w-32 h-32 mx-auto mb-4">
            <svg className="w-32 h-32 transform -rotate-90" viewBox="0 0 36 36">
              ...
              />
              <path
                className={getScoreColor(score_category)}
                stroke="currentColor"
                strokeWidth="3"
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <Badge
              variant={
                score_category.toLowerCase() === "high"
                  ? "default"
                  : score_category.toLowerCase() === "medium"
                    ? "secondary"
                    : "destructive"
              }
            >
