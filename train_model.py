...
def generate_and_save_data():
    if os.path.exists('customer_feedback_satisfaction.csv'):
        print("➡️ Dataset 'customer_feedback_satisfaction.csv' already exists. Skipping generation.")
        return
    print("⏳ Generating sample dataset: customer_feedback_satisfaction.csv...")
    np.random.seed(42)
    num_samples = 1000
    data = {'CustomerID': range(1, num_samples + 1), 'Age': np.random.randint(18, 70, size=num_samples), 'Income': np.random.randint(25000, 150000, size=num_samples), 'ProductQuality': np.random.randint(1, 11, size=num_samples), 'ServiceQuality': np.random.randint(1, 11, size=num_samples), 'PurchaseFrequency': np.random.randint(1, 51, size=num_samples), 'Gender': np.random.choice(['Male', 'Female'], size=num_samples), 'Country': np.random.choice(['USA', 'Canada', 'UK', 'Germany', 'France'], size=num_samples), 'FeedbackScore': np.random.choice(['Low', 'Medium', 'High'], size=num_samples), 'LoyaltyLevel': np.random.choice(['Bronze', 'Silver', 'Gold', 'Platinum'], size=num_samples)}
